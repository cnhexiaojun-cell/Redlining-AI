  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.includes("Invalid or expired captcha") || msg.includes("captcha")) {
        ...
        loadCaptcha();
      } else if (msg.includes("Password must be") || msg.includes("letters and numbers")) {
        setError(t("auth_error_password_rules"));
      } else {
        setError(msg || t("auth_register"));
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">{t("auth_register_title")}</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {submitting ? t("auth_submitting") : t("auth_register")}
          </button>
        </form>
